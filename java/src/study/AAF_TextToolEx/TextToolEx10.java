...
/* 190731.
 * [문제10] TextArea의 데이터를 라인별로 읽어서 param1에 입력된 형식에 맞게 변형하여 보여주는 '패턴적용'버튼을 구현하세요.
[출처] [Java1000제] Text데이터 편집도우미10 - 패턴적용 (남궁성의 코드초보스터디(자바 java, c언어, javascript, python) |작성자 남궁성
*/
/* 테스트 데이터
남궁성@1 
강석윤@2 
		...
		btn[n++].addActionListener(new ActionListener() { // 패턴적용
			public void actionPerformed(ActionEvent ae) {
				...
				for (int i = 0; s.hasNextLine(); i++) {
					...
					String fmtLine = MessageFormat.format(pattern, text);
					sb.append(fmtLine);
					sb.append(CR_LF);
				}
				ta.setText(sb.toString());
