...
[출처] [Java1000제] Text데이터 편집도우미6 - 접두사, 접미사 삭제 (남궁성의 코드초보스터디(자바 java, c언어, javascript, python) |작성자 남궁성
*/
/* 테스트 데이터
sb.append("aaaaaa");
	...
	private void registerEventHandler() {
		...
		btn[n++].addActionListener(new ActionListener() { // substring - 문자열 자르기
			public void actionPerformed(ActionEvent ae) {
				...
				int paramSize1 = param1.length();
				int paramSize2 = param2.length();
				Scanner s = new Scanner(curText);
				while(s.hasNextLine()) {
					String line = s.nextLine();
					String text = line.substring(paramSize1, line.length()- paramSize2);
