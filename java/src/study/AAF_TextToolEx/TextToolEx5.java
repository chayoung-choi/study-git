...
[출처] [Java1000제] Text데이터 편집도우미5 - 접두사, 접미사 붙이기 (남궁성의 코드초보스터디(자바 java, c언어, javascript, python) |작성자 남궁성
*/
/* 테스트 데이터
	...
	private void registerEventHandler() {
		...
		btn[n++].addActionListener(new ActionListener() { // 접두사 - 각 라인에 접두사, 접미사 붙이기
			public void actionPerformed(ActionEvent ae) {
				...
				Scanner s = new Scanner(curText);
				while(s.hasNextLine()) {
					sb.append(param1).append(s.nextLine()).append(param2).append(CR_LF);
				}
				ta.setText(sb.toString());
			}
		});
